...
# Diccionarios de datos para almacenar clientes, productos y pedidos
clientes = {}
...
def ver_clientes():
    ...
    for id_cliente, dato in clientes.items():
        print("ID:", id_cliente, "Nombre:", dato['nombre'], "Dirección:", dato['direccion'], "Email:", dato['email']) # Muestra los datos de cada cliente "despiezando" solo los items del diccionario que estaba relacionado con el ID del cliente que ya está registrado

# Función para realizar una compra
def realizar_compra():
    espacio()
    print("--- 3 Realizar Compra ---")
    id_clientecompra = int(input("Ingrese el ID del cliente que realiza la compra: "))
    
    if id_clientecompra not in clientes: # Comprueba que el id de clientes se encuentre en el diccionario clientes donde se almacenan todos los ID
    ...
    while True: # Bucle para comprar los productos e indicar la cantidad que quieres de ellos, lo cual multiplica el precio
        ...
        if producto == fin: # Condicional que termina el bucle de compra
            break
        if producto in productos: # Función que pide la cantidad de producto y actualiza el precio correspondiente al producto y la cantidad, primero comprueba que el producto exista en la lista "productos" creada al principio
            cantidad = int(input("Ingrese la cantidad: "))
            productos_comprados.append((producto, cantidad)) # Añadimos el producto a la nueva lista de la compra
    ...
    # Creamos un subdiccionario al que asociamos el número de pedido, y en este se encuentran todos los datos del cliente
    pedidos[numero_pedido] = {"id_cliente": id_clientecompra, "productos": productos_comprados, "total": total_compra} # Almacena el Id del pedido en el diccionario pedidos relacionando el número de pedido con el id del cliente tal como registramos a los clientes
